...
let User = require('../domains/users');
...
UserRepository.prototype = {
  ...
  findAll: function(cb, errCb){
    let db = this.db;
    let query = 'SELECT * FROM user, media WHERE user.id=media.id';
    db.query(query, (err, results, fields) => {
      ...
      for(let i=0;i<results.length;i++){
        let e = results[i];
        let employee = new User(e.id, e.nama, e.alamat, e.notlp, e.status, e.description);
